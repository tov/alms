...
instance Tag i ⇒ HasAnnotations (Type' i) i where
  annotFtvMapM con t0 = case t0 of
    ...
    [ty|'  '$tv |]                → afm con tv
    [ty|' $t1 -[$opt:qe]> $t2 |]  → do
      t1m ← mapM (con (qident "->") 1) =<< afm con t1
      qem ← mapM (con (qident "->") 2) =<< afm con qe
      t2m ← mapM (con (qident "->") 3) =<< afm con t2
